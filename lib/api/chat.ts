...
import { Conversations, GetMessagesResponse, Messages } from "@/lib/types"
...
export async function sendMessage(payload: Partial<Messages>): Promise<Messages> {
	const res = await api.post<Messages>("/sb/chat", payload)
	return res.data
}
